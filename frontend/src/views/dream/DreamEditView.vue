...
const dreamId = ref<number>(Number(route.params.id))
...
onMounted(async () => {
  try {
    const dream = await dreamStore.fetchDream(dreamId.value)
    if (dream) {
      form.title = dream.title
      form.content = dream.content
      form.dreamDate = dream.dreamDate
      form.mood = dream.moodBeforeSleep || ''
      form.sleepQuality = dream.sleepQuality || 'FAIR'
      form.tags = dream.tags || []
      form.visibility = dream.isPrivate ? 'private' : 'public'
    }
  } catch (error: any) {
    errorMessage.value = error.response?.data?.message || '加载梦境详情失败'
  } finally {
...
async function handleSubmit() {
  ...
  try {
    await dreamStore.updateDream(dreamId.value, {
    ...
    router.push(`/dreams/${dreamId.value}`)
  } catch (error: any) {
...
<template>
  <div class="dream-edit-view">
    <div class="detail-header-spacer"></div>
    
    <main class="dream-edit-view__main">
      <div class="container">
        <!-- 加载状态 -->
        <div v-if="initialLoading" class="dream-edit-view__loading">
          <div class="loading-spinner"></div>
          <p>加载中...</p>
        </div>
        
        <SakuraCard v-else variant="glass" padding="lg" class="dream-edit-view__card">
          <header class="dream-edit-view__header">
            <h1 class="dream-edit-view__title">✏️ 编辑梦境</h1>
            <p class="dream-edit-view__subtitle">修改你的梦境记录</p>
          </header>
          ...
          <form class="dream-edit-view__form" @submit.prevent="handleSubmit">
            <!-- 标题 -->
            <SakuraInput
              v-model="form.title"
              label="梦境标题"
            ...
            <!-- 按钮 -->
            <div class="dream-edit-view__actions">
              <SakuraButton 
                variant="ghost" 
                type="button"
                @click="handleCancel"
